...
//    private ItemHandler handler;
    ...
    public void setImage(BufferedImage image, String imagePath) {
        ...
        imgData = FactoryImageProcess.imageToPixelsInt(currBufferedImage);
...
//            int py = 30;
            if (activateAutoSize) {
                currBufferedImage = FactoryImageProcess.resize(originalBufferedImage, this.getWidth(), this.getHeight());
            }
            if (activateAutoSizeAspect) {
                currBufferedImage = FactoryImageProcess.resizeAspectRatio(originalBufferedImage, this.getWidth(), this.getHeight());
            }
...
//                int gray=imgMatrix[p.x][p.y];
    ...
    private void checkPrevButton(MouseEvent e) {
        ...
        if (e.getPoint().x > tx && e.getPoint().x < tx + 75 && e.getPoint().y > ty && e.getPoint().y < ty + 30) {
            ...
            currBufferedImage = FactoryImageProcess.readImageFromFile(fileList[currentImageIndex]);
    ...
    private class ItemHandler implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (obj.getText().equals("Clone ROI")) {
                ...
                BufferedImage bf = FactoryImageProcess.pixelsToImageGray(cm.toIntArray2D());
            ...
            if (obj.getText().equals("Histogram")) {
                ...
                imgData = FactoryImageProcess.imageToPixelsInt(originalBufferedImage);
                currBufferedImage = FactoryImageProcess.pixelsToImageGray(imgData);
                imgData = FactoryImageProcess.imageToPixelsInt(currBufferedImage);
                if (histFrame == null) {
...
//                currBufferedImage = ImageProcess.pixelsToBufferedImage255(ImageProcess.imageToPixels255(originalBufferedImage));
            ...
            if (obj.getText().equals("Edge")) {
                setDefaultValues();
                activateEdge = true;
                currBufferedImage = FactoryImageProcess.pixelsToImageGray(FactoryImageProcess.imageToPixelsInt(originalBufferedImage));
