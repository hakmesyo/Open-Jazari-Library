...
public class PanelPicture extends JPanel {
    ...
    public void setImage(BufferedImage image) {
        ...
        imgData = FactoryImageProcess.imageToPixelsDouble(currBufferedImage);
        lbl.setText(getImageSize() + "X:Y");
        if (activateStatistics) {
            currBufferedImage = FactoryImageProcess.toGrayLevel(originalBufferedImage);
            imgData = FactoryImageProcess.imageToPixelsDouble(currBufferedImage);
    ...
    public void paint(Graphics gr) {
        ...
        if (currBufferedImage != null) {
            if (activateAutoSize) {
                currBufferedImage = FactoryImageProcess.resize(originalBufferedImageTemp, this.getWidth() - 2 * panWidth, this.getHeight() - 2 * panWidth);
                imgData = FactoryImageProcess.imageToPixelsDouble(currBufferedImage);
                lbl.setText(getImageSize() + "X:Y");
//                gr.drawImage(currBufferedImage, fromLeft, fromTop, this);
            } else if (activateAutoSizeAspect) {
                currBufferedImage = FactoryImageProcess.resizeAspectRatio(originalBufferedImageTemp, this.getWidth() - 2 * panWidth, this.getHeight() - 2 * panWidth);
                imgData = FactoryImageProcess.imageToPixelsDouble(currBufferedImage);
                lbl.setText(getImageSize() + "X:Y");
//                gr.drawImage(currBufferedImage, fromLeft, fromTop, this);
            }
...
//                System.out.println("row:"+(mousePos.y-fromTop)+" col:"+(mousePos.x-fromLeft));
    ...
    public void saveImage() {
        FactoryImageProcess.imwrite(currBufferedImage);
    }

    public void saveImage(String fileName) {
        FactoryImageProcess.imwrite(currBufferedImage, fileName);
    }
    ...
    private class ItemHandler implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (obj.getText().equals("Load Image")) {
                ...
                File fl = FactoryImageProcess.readImageFileFromFolderWithDirectoryPath(imagePath);
                if (fl == null) {
                    return;
                }
                BufferedImage bf = FactoryImageProcess.readImageFromFile(fl.getAbsolutePath());
                if (bf != null) {
...
//                histFrame.setVisible(true);
            ...
            if (obj.getText().equals("Red")) {
                ...
                currBufferedImage = FactoryImageProcess.isolateChannel(originalBufferedImage, "red");
                originalBufferedImageTemp = FactoryImageProcess.clone(currBufferedImage);
                repaint();
                return;
            }
            if (obj.getText().equals("Green")) {
                setDefaultValues();
                activateRedChannel = true;
                currBufferedImage = FactoryImageProcess.isolateChannel(originalBufferedImage, "green");
                originalBufferedImageTemp = FactoryImageProcess.clone(currBufferedImage);
                repaint();
                return;
            }
            if (obj.getText().equals("Blue")) {
                setDefaultValues();
                activateRedChannel = true;
                currBufferedImage = FactoryImageProcess.isolateChannel(originalBufferedImage, "blue");
                originalBufferedImageTemp = FactoryImageProcess.clone(currBufferedImage);
                repaint();
                return;
            }
            ...
            if (obj.getText().equals("Gray")) {
                setDefaultValues();
                activateGray = true;
                currBufferedImage = FactoryImageProcess.rgb2gray(originalBufferedImage);
                originalBufferedImageTemp = FactoryImageProcess.clone(currBufferedImage);
                imgData = FactoryImageProcess.imageToPixelsDouble(currBufferedImage);
                repaint();
                return;
            ...
            if (obj.getText().equals("HSV")) {
                setDefaultValues();
                activateHSV = true;
                currBufferedImage = FactoryImageProcess.toHSVColorSpace(originalBufferedImage);
                originalBufferedImageTemp = FactoryImageProcess.clone(currBufferedImage);
//                imgData = ImageProcess.imageToPixels255Double(currBufferedImage);
                repaint();
                return;
            ...
            if (obj.getText().equals("Edge")) {
                setDefaultValues();
                activateEdge = true;
                currBufferedImage = FactoryImageProcess.edgeDetectionCanny(currBufferedImage, 0.3f, 1.0f, 2.5f, 3, false);
                originalBufferedImageTemp = FactoryImageProcess.clone(currBufferedImage);
                imgData = FactoryImageProcess.imageToPixelsDouble(currBufferedImage);
                repaint();
                return;
...
//            }
            if (obj.getText().equals("Revert")) {
                setDefaultValues();
                activateRevert = true;
                currBufferedImage = FactoryImageProcess.revert(currBufferedImage);
                originalBufferedImageTemp = FactoryImageProcess.clone(currBufferedImage);
                repaint();
                return;
            }
...
//                CMatrix cm = CMatrix.getInstance(currBufferedImage).subMatrix(mousePosTopLeft, mousePosBottomRight);
                ...
                BufferedImage bf = FactoryImageProcess.cropImage(currBufferedImage, cr);
