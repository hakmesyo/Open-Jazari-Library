...
public class MatrixUtils {
    ...
    public static Matrix random(int rows, int cols) {
        ...
        CMatrix cm = CMatrix.getInstance().rand(rows, cols,rnd);
...
//        }
    ...
    public static Matrix softmax(Matrix m) {
        ...
        double[][] d=m.data.toDoubleArray2D();
        int r=d.length;
        int c=d[0].length;
        double max = 0.0;
        double sum=0;
        for (int i = 0; i < r; i++) {
            ...
            double[] d1=FactoryUtils.subtract(d[i],max);
            double[] exp=FactoryUtils.exp(d1);
            sum=FactoryUtils.sum(exp);
            double[] ret=FactoryUtils.divide(exp,sum);
            d[i]=ret;
        }
