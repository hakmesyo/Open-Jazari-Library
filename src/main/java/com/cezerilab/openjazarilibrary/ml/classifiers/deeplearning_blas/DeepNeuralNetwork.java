...
//    }
    ...
    private FeedForwardResult[] feedForward(Matrix x, Matrix[] dropoutMasks) {
        ...
        for (int layer = 1; layer < numLayers; layer++) {
            ...
            if (layerParams[layer].isConvolutional()) {
                ...
                ffr[layer].output = ffr[layer].input.trans2mult(thetas[layer]);
    ...
    private double getCost(Matrix x, Matrix y, double weightPenalty, int batchSize) {
        ...
        if (weightPenalty > 0) {
            ...
            double regSum = 0.0;
            for (int t = 1; t < thetas.length; t++) {
                for (MatrixElement me: thetas[t].getColumns(1, -1)) {
...
//        }
        ...
        for (int layer = numLayers-2; layer >=1; layer--) {
            delta[layer] = delta[layer+1].mult(thetas[layer+1]).getColumns(1, -1);
            ...
            Matrix temp2=ffr[layer].input.trans2mult(thetas[layer]);
            delta[layer].multElements(MatrixUtils.rectifyGradient(temp2), delta[layer]);
        }
                ...
                if ((l == 0 && params.inputLayerDropoutRate > 0) || (l > 0 && params.hiddenLayersDropoutRate > 0)) {
                    ...
                    for (int i = 0; i < numExamples; i++) {
                        for (int j = 0; j < numColumns; j++) {
                            mask.myMatrix.set(i,j,rnd.nextDouble() > dropoutRate ? 1.0 : 0.0);
                        }
                    }
