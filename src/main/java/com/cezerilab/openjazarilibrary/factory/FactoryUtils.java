...
import com.cezerilab.openjazarilibrary.types.TLearningType;
...
//        writeToFile(file_path, s);
    ...
    public static String readFile_FirstLine(String fileName) {
        String ret = "";
        File file = new File(fileName);
        if (!file.exists()) {
            showMessage(fileName + " isminde bir dosya yok");
...
//        return transpose(ret);
     ...
     * @return 2D double array
     */
    public static float[][] readCSV_Float(String filename, char separator, int headerLines) {
        BufferedReader br = null;
        java.util.List<String[]> values = null;
        try {
            br = new BufferedReader(new FileReader(filename));
    ...
    public static List<String> readCSV_featureNames(String filename, char separator) {
        String s=readFile_FirstLine(filename);
        List<String> ret=new ArrayList();
        if (s!=null && !s.isEmpty()) {
            String[] str=s.split(separator+"");
            ret=Arrays.asList(str);
        }
        return ret;
    }
